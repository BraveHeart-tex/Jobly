...
const LondonTemplate = ({ data }: LondonTemplateProps) => {
	...
	const renderSections = () => {
		const sections = Object.values(
			exclude(data, ["personalDetailsSection"]),
		);
		...
		return sections
			...
			.map((section) => {
				...
				if (
					section.internalSectionTag === INTERNAL_SECTION_TAGS.SKILLS
				) {
					return (
						<SkillsSection
							skillsSection={
								section as MakeResumeDataReturn["skillsSection"]
							}
						/>
					);
				}
				...
				if (
					section.internalSectionTag === INTERNAL_SECTION_TAGS.CUSTOM
				) {
					return (
						<CustomResumeSection
				...
				if (
					section.internalSectionTag === INTERNAL_SECTION_TAGS.HOBBIES
				) {
					return (
						<HobbiesSection
							hobbiesSection={
								section as MakeResumeDataReturn["hobbiesSection"]
							}
						/>
					);
				}
				...
				if (
					section.internalSectionTag === INTERNAL_SECTION_TAGS.COURSES
				) {
					return (
						<CoursesSection
							coursesSection={
								section as MakeResumeDataReturn["coursesSection"]
							}
						/>
					);
				}
...
const SkillsSection = ({
	...
	const { shouldRenderSkillsSectionItems, skillsSectionItems } =
		skillsSection;
	return (
		<>
			{/* Skills */}
			...
			{shouldRenderExtraCurricularActivitiesSectionItems ? (
				<View
					style={{
						...
						display: "flex",
						flexDirection:
							extraCurricularActivitiesSectionItems.length ===
								1 &&
...
const CustomResumeSection = ({
	...
	const { shouldRenderCustomSectionItems, customSectionItems } =
		customSection;
