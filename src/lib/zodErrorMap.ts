...
export const zodErrorMap: ZodErrorMap = (issue, _ctx) => {
  switch (issue.code) {
    ...
    case ZodIssueCode.unrecognized_keys:
      return { message: `Unrecognized keys in object: ${issue.keys.join(", ")}` };
    case ZodIssueCode.invalid_union:
      return { message: `Invalid input, expected one of the union types` };
    case ZodIssueCode.invalid_enum_value:
      return { message: `Invalid enum value. Expected one of: ${issue.options.join(", ")}` };
    ...
    case ZodIssueCode.too_small:
      if (issue.type === "string") {
        return { message: `Should at least be ${issue.minimum} character${issue.minimum === 1 ? "" : "s"}` };
      }
      return { message: `Value is too small. Minimum allowed is ${issue.minimum}` };
    case ZodIssueCode.too_big:
      if (issue.type === "string") {
        return { message: `Can at most be ${issue.maximum} ${"character" + issue.maximum === "1" ? "" : "s"}` };
      }
      return { message: `Value is too big. Maximum allowed is ${issue.maximum}` };
